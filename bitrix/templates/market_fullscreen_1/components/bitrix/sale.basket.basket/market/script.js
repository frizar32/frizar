	...
	if (basketItemId !== null && !!res.BASKET_DATA)
	{
			...
			switch (arColumns[i])
			{
				...
				case 'QUANTITY':
					...
					oCellQuantityHTML += '<td>';
                                        ...
                                        if (ratio != 0	&& ratio != '')
						oCellQuantityHTML += '<a href="javascript:void(0);" class="minus bxr-quantity-button-minus-big-basket" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'down\', ' + bUseFloatQuantity + ');">-</a>';
					...
					oCellQuantityHTML += '<input type="text" size="3" id="QUANTITY_INPUT_' + arItem['ID'] + '"\
						...
						>';
                                        ...
                                        if (ratio != 0	&& ratio != '')
                                            oCellQuantityHTML += '<a href="javascript:void(0);" class="minus bxr-quantity-button-plus-big-basket" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'down\', ' + bUseFloatQuantity + ');">+</a>'
		...
		if (bShowDeleteColumn || bShowDelayColumn)
		{
			...
			if (bShowDeleteColumn)
				oCellControl.innerHTML = '<a class="bxr-icon-button-delete-big-baske" href="' + basketJSParams['DELETE_URL'].replace('#ID#', arItem['ID']) +'"><span class="fa fa-close" aria-hidden="true"></span></a>';

			if (bShowDelayColumn)
				oCellControl.innerHTML += '<a  class="bxr-icon-button-delay-big-baske"  href="' + basketJSParams['DELAY_URL'].replace('#ID#', arItem['ID']) + '"><span class="fa fa-heart-o" aria-hidden="true"></span></a>';
		}
	...
	if (!!res.BASKET_DATA)
	{
		...
		if (BX('PRICE_WITHOUT_DISCOUNT'))
			BX('PRICE_WITHOUT_DISCOUNT').innerHTML = (res['BASKET_DATA']['PRICE_WITHOUT_DISCOUNT'] != res['BASKET_DATA']['allSum_FORMATED']) ? res['BASKET_DATA']['PRICE_WITHOUT_DISCOUNT'].replace(/\s/g, '&nbsp;') : '';
                ...
                //min order price block toggle
                minOrderPrice = parseInt($('#min-order-price').val());
                currency = $('#currency-format').val();
                minOrderPriceFormated = currency.replace("#", minOrderPrice);
                addPrice = minOrderPrice-res['BASKET_DATA']['allSum'];
                addPrice = addPrice.toFixed(2);
                addPriceFormated = currency.replace("#", addPrice);
                minOrderPriceMsg = $('#min-order-price-msg').val();
                minOrderPriceMsg = minOrderPriceMsg.replace("#MIN_ORDER_PRICE#", minOrderPriceFormated);
                minOrderPriceMsg = minOrderPriceMsg.replace("#ADD_ORDER_PRICE#", addPriceFormated);
                if (res['BASKET_DATA']['allSum'] >= minOrderPrice) {
                    $('.min-order-price-notify').hide();
                    $('.checkout').show();
                } else {
                    $('.min-order-price-notify').html(minOrderPriceMsg);
                    $('.min-order-price-notify').show();
                    $('.checkout').hide();
                }
...
{
        var ratiostring = String(ratio);
        var ratioseparated = ratiostring.split('.');
		var ratiolength = 0;
		if ((ratioseparated[1] != undefined) && (ratioseparated[1] != null) && (ratioseparated[1] != '')){
			ratiolength = ratioseparated[1].length;
		}
		else {
			ratiolength = 0;
		}
	...
	newVal = (bUseFloatQuantity === false && bIsQuantityFloat === false) ? parseInt(newVal) : parseFloat(newVal).toFixed(ratiolength);
...
function setQuantity(basketId, ratio, sign, bUseFloatQuantity)
{
	...
	{
		newVal = newVal.toFixed(ratiolength);
	}
	...
	if (!bUseFloatQuantity && newVal != newVal.toFixed(ratiolength))
	{
		newVal = newVal.toFixed(ratiolength);
	}
...
{    
	var ratiostring = String(ratio);
        ...
        var newValInt = parseInt(quantity * 10000),
		ratioInt = parseInt(ratio * 10000),
		reminder = newValInt % ratioInt,
		result = quantity,
	...
	if (ratio !== 0 && ratio != 1)
	{
            for (i = ratio, max = parseFloat(quantity)+parseFloat(ratio); i < max; i = parseFloat(parseFloat(i) + parseFloat(ratio)).toFixed(ratiolength))
            {
                result = i;
            }
	}
	...
	result = (bUseFloatQuantity === false && bIsQuantityFloat === false) ? parseInt(result, 10) : parseFloat(result).toFixed(ratiolength);
