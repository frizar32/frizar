...
if($arParams['CATALOG_SUBSCRIBE'] == 'Y')
    $showSubscribeBtn = true;
else
    $showSubscribeBtn = false;
$usePriceCount = ('Y' == $arParams['USE_PRICE_COUNT']);
global $ratio_settings,$bxr_ratio_prop_code;?>
<div class="padding_brdr">
	<div id="bxr-market-price-wrap" itemprop="offers" itemscope <?=(count($arResult["OFFERS"]) > 0)?'itemtype="https://schema.org/AggregateOffer"':'itemtype="https://schema.org/Offer"'?>>
...
<?  endforeach;
  ...
  <style>
      ...
      #bxr-market-detail-basket-btn-wrap .btns-row  .bx-catalog-subscribe-button{
          margin-bottom: 0;
      }
  </style>
  <?if($arResult['PROPERTIES']['SNYAT_S_PROIZVODSTVA']['VALUE']){?>
      <div class="btns-row">
        <span class="bxr-color-button not-clickcable"><?=$arResult['PROPERTIES']['SNYAT_S_PROIZVODSTVA']['NAME'] ?></span>

        <?if($arParams["SHOW_ANALOG_BTN"]):?>
            <script>
                trade_name = "<?=$arResult['NAME']?>";
                trade_id = "<?=$arResult['ID']?>";
                trade_link = "<?=$arResult['~DETAIL_PAGE_URL']?>";
                formRequestMsgAnalog = "<?=GetMessage('TRADE_ANALOG_MSG')?>";
                formRequestMsgAnalog = formRequestMsgAnalog.replace("#TRADE_NAME#",'<?=htmlspecialchars($arResult['NAME'],ENT_QUOTES, SITE_CHARSET)?>');
                formRequestMsgReq = "<?=GetMessage('TRADE_REQUEST_MSG')?>";
                formRequestMsgReq = formRequestMsgReq.replace("#TRADE_NAME#",'<?=htmlspecialchars($arResult['NAME'],ENT_QUOTES, SITE_CHARSET)?>');
            </script>
            <button class="bxr-color-button bxr-analog" value="<?=$arResult["ID"]?>">Подобрать аналог</button>
        <?endif;?>
  ...
  <?}else{?>
    ...
    <?php } else {
        ...
        <?}?>
        <form class="bxr-basket-action bxr-basket-group bxr-currnet-torg">
            <div class="group_input"><input type="button" class="bxr-quantity-button-minus" value="-" data-item="<?=$arResult["ID"]?>" data-ratio="<?=$ratio;?>">
            <input type="text" name="quantity" value="<?=$quantity;?>" class="bxr-quantity-text" data-item="<?=$arResult["ID"]?>">
            <input type="button" class="bxr-quantity-button-plus" value="+" data-item="<?=$arResult["ID"]?>" data-ratio="<?=$ratio;?>" data-max="<?=$qtyMax?>"></div>
            <button class="bxr-color-button bxr-basket-add" onclick="yaCounter49057298.reachGoal('add_to_cart'); return true;">
            <?
            /*if ($arResult["CATALOG_QUANTITY"] > 0): */
    		    if (strlen($arParams["MESS_BTN_BUY"]) > 0):
	...
	<div class="bxr-share-icon-wrap">
	    ...
	    <?$APPLICATION->IncludeComponent(
	            ...
	            Array(
	                    ...
	                    "HANDLERS" => array("delicious","twitter","vk","pinterest","google"),
	                    "HIDE" => "N",
        ...
        <?if($arResult['PRODUCT']['QUANTITY'] > 0):?>
          <div class="bxr-props-table">
            <div class="bxr-props-row">
                <div class="bxr-props-name"><span>Есть на складе: </span></div>
                <div class="bxr-props-data"><span><?=$arResult['PRODUCT']['QUANTITY']?> шт</span></div>
            </div>
          </div>
        <?endif;?>
