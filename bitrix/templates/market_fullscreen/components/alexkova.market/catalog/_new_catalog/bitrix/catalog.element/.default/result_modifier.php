...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	if (count($arResult["OFFERS"])>0){
		foreach($arResult["OFFERS"] as &$arItemOffer){
                    $arItemOffer["DETAIL_PAGE_URL"] = $arResult["DETAIL_PAGE_URL"].$arParams["OFFER_MASK"]."/".$arItemOffer["ID"]."/";
			$arItemOffer["CAN_BUY"] = true;
			if ($arItemOffer["CATALOG_QUANTITY"]<=0 && ($arItemOffer["CATALOG_CAN_BUY_ZERO"] == "N" || $arItemOffer["CATALOG_NEGATIVE_AMOUNT_TRACE"] == "N")){
				$arItemOffer["CAN_BUY"] = false;
			};
		}
	}else{
		if ($arResult["CATALOG_QUANTITY"]<=0 && ($arResult["CATALOG_CAN_BUY_ZERO"] == "N" || $arResult["CATALOG_NEGATIVE_AMOUNT_TRACE"] == "N")){
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		$arOffer['MORE_PHOTO_COUNT'] = 0;
                if ($arParams['ADD_DETAIL_TO_SLIDER_SKU'] == Y || $arParams['ADDITIONAL_SKU_PIC_2_SLIDER'] == Y || $arParams['FILTER_SKU_PHOTO'] == Y) {
	...
	$arResult['OFFERS_PROP_CODES'] = (!empty($arUsedFields) ? base64_encode(serialize(array_keys($arUsedFields))) : '');
        ...
        if(isset($arParams["SKU_SORT_PARAMS"]) && $arParams["SKU_SORT_PARAMS"] == "Y")
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		if (count($arOffer["DISPLAY_PROPERTIES"])>0) $arResult['SHOW_OFFERS_PROPERTY'] = true;
                ...
                foreach ($arOffer["PRICES"] as $offerPriceIndex => $offerPrice) {
                    if (empty($arResult['MIN_PRICE'][$offerPriceIndex]) && $arOffer['CAN_BUY'])
                    {
                        $intSelected = $keyOffer;
                        $arResult['MIN_PRICE'][$offerPriceIndex] = $offerPrice;
                    } elseif ($arOffer['CAN_BUY']) {
                        $arResult['MIN_PRICE'][$offerPriceIndex] = ($offerPrice['VALUE'] < $arResult['MIN_PRICE'][$offerPriceIndex]['VALUE']) ? $offerPrice : $arResult['MIN_PRICE'][$offerPriceIndex];
                    }
                }
		$arSKUProps = false;
...
if ($usePriceCount) {
    foreach ($arResult['OFFERS'] as $key => $offer) {
        foreach ($offer['PRICES'] as $priceCode => $price) {
            $currency = ($price["CURRENCY"] == "RUB") ? GetMessage('RUB') : $price["CURRENCY"];
            $dbPrice = CPrice::GetList(
                    array("QUANTITY_FROM" => "ASC", "QUANTITY_TO" => "ASC", "SORT" => "ASC"),
                    array("PRODUCT_ID" => $offer["ID"], "CATALOG_GROUP_ID" => $price["PRICE_ID"]),
                    false,
                    false,
...
//                $arResult['OFFERS'][$key]['PRICES'][$priceCode]["DISCOUNT_VALUE"] = $discountPrice;
//                $arResult['OFFERS'][$key]['PRICES'][$priceCode]["PRINT_DISCOUNT_VALUE"] = CurrencyFormat($discountPrice,$price["CURRENCY"]);
            }
        }
    }
}
        ...
        foreach ($offer["PROPERTIES"] as $propCode => $prop) {
            if (in_array($propCode, $sku_props_code)) {
                ...
                if ($prop["PROPERTY_TYPE"] == "S") {
                    ...
                    if (!in_array($prop["VALUE"], $elem_hl[$propCode]["VALUES"]))
...
if( (isset($arParams["DETAIL_DISPLAY_SHOW_FILES"]) && $arParams["DETAIL_DISPLAY_SHOW_FILES"] == "Y") ||
  (isset($arParams["DETAIL_DISPLAY_SHOW_VIDEO"]) && $arParams["DETAIL_DISPLAY_SHOW_VIDEO"] == "Y")  )
{
    $res = CIBlockSection::GetNavChain($arResult["IBLOCK_ID"],  $arResult["IBLOCK_SECTION_ID"], array("ID", "IBLOCK_SECTION_ID"));
    while($ar_section = $res->GetNext())
    {
        $section_id_chain[] = $ar_section["ID"];
    }

    $arResult["PROPERTIES"]["UF_VIDEO"] = array();
    $arResult["PROPERTIES"]["UF_FILES"] = array();


    if(!empty($section_id_chain)) {
        $res = CIBlockSection::GetList(array(), array("IBLOCK_ID" => $arResult["IBLOCK_ID"], "ID" => $section_id_chain), false, array("UF_VIDEO", "UF_FILES"));
        while($ar_section = $res->GetNext())
        {

            if(!empty($ar_section["UF_VIDEO"])){
                foreach ($ar_section["~UF_VIDEO"] as  $k => $val) {
                    $ar_section["~UF_VIDEO"][$k] = unserialize($val);
                }

                if(!empty($ar_section["~UF_VIDEO"])) {
                    $arResult["PROPERTIES"]["UF_VIDEO"] = array_merge($arResult["PROPERTIES"]["UF_VIDEO"], $ar_section["~UF_VIDEO"]);
                }
            }

            if(!empty($ar_section["UF_FILES"]) && !empty($ar_section["UF_FILES"][0])){
                $arResult["PROPERTIES"]["UF_FILES"] = array_merge($arResult["PROPERTIES"]["UF_FILES"], $ar_section["UF_FILES"]);
            }
        }

        $arResult["PROPERTIES"]["UF_VIDEO"] = array_reverse($arResult["PROPERTIES"]["UF_VIDEO"], true);
        $arResult["PROPERTIES"]["UF_FILES"] = array_reverse($arResult["PROPERTIES"]["UF_FILES"], true);
...
/*Show tabs*/
    ...
    if (count($arResult["OFFERS"]) > 0) {
        $arTabs["offers"]["include_top"][] = "script.php";
        if ($arParams["HIDE_OFFERS_LIST"] != "Y"){
            $arTabs["offers"]["include"][] = "offers_list.php";
            $arTabs["offers"]["name"] = GetMessage("OFFERS_TEXT");
            $arTabs["offers"]["class"] = "bxr-detail-offers";
        }
    }
    ...
    if (count($arResult["DISPLAY_PROPERTIES"]) > 0) {
        $arTabs["props"]["include"][] = "properties_list.php";
        $arTabs["props"]["name"] = GetMessage("PROPS_TEXT");
        $arTabs["props"]["class"] = "bxr-detail-props";
    }

    if ($arResult["DETAIL_TEXT"]) {
        $arTabs["detail"]["include"][] = "detail_text.php";
        $arTabs["detail"]["name"] = GetMessage("DETAIL_TEXT");
        $arTabs["detail"]["class"] = "bxr-detail-text";
		$arTabs["detail"]["echo"][] = '<meta itemprop="description" content="'.substr(strip_tags($arResult["DETAIL_TEXT"]), 0, 200).'">';
    }
    else {
        $arTabs["detail"]["echo"][] = '<meta itemprop="description" content="'.$arResult["NAME"].'">';
    }

    $arTabs["delivery"]["include"][] = "delivery_text.php";
    $arTabs["delivery"]["name"] = "Оплата и доставка";
    $arTabs["delivery"]["class"] = "bxr-detail-text";

    if( isset($arParams["DETAIL_DISPLAY_SHOW_FILES"]) && $arParams["DETAIL_DISPLAY_SHOW_FILES"] == "Y" &&
     (!empty($arResult["PROPERTIES"]["FILES"]["VALUE"]) || !empty($arResult["PROPERTIES"]["UF_FILES"]))) {
        $arTabs["element-files"]["include"][] = "files.php";
        $arTabs["element-files"]["name"] = $arResult["PROPERTIES"]["FILES"]["NAME"];
        $arTabs["element-files"]["class"] = "bxr-detail-files";
    }

    if(isset($arParams["DETAIL_DISPLAY_SHOW_VIDEO"]) && $arParams["DETAIL_DISPLAY_SHOW_VIDEO"] == "Y") {
        if((!empty($arResult["PROPERTIES"]["UF_VIDEO"])) || (!empty($arResult["PROPERTIES"]["VIDEO"]["VALUE"]) && !empty($arResult["PROPERTIES"]["VIDEO"]["VALUE"][0]))) {
            $arTabs["element-video"]["include"][] = "video.php";
            $arTabs["element-video"]["name"] = $arResult["PROPERTIES"]["VIDEO"]["NAME"];
            $arTabs["element-video"]["class"] = "bxr-detail-video";

            if($arParams["VIDEO_PLAYER"]=="IFRAME")
                $arTabs["element-video"]["id"] = "container-video-iframe";
            elseif($arParams["VIDEO_PLAYER"]=="MEJ")
                $arTabs["element-video"]["id"] = "container-video-mej";
        }
    }

    if ($arParams["USE_COMMENTS"] == "Y") {
        $arTabs["review"]["include"][] = "reviews.php";
        $arTabs["review"]["name"] = GetMessage("REVIEWS_TEXT");
        $arTabs["review"]["epilog"] = true;
        $arTabs["review"]["class"] = "bxr-detail-review";
    }

    $storeAmount = ('Y' == $arParams['USE_STORE']);
    if ($storeAmount) {
        $arTabs["storeamount"]["include"][] = "store_amount.php";
        $arTabs["storeamount"]["name"] = GetMessage("STORE_AMOUNT_TEXT");
        $arTabs["storeamount"]["class"] = "bxr-detail-store-amount";
    }

    if ($arParams["ADDITIONAL_TAB_SHOW"] == 'Y' && !empty($arParams["ADDITIONAL_TAB_PATH"]) && file_exists($_SERVER["DOCUMENT_ROOT"].$arParams["ADDITIONAL_TAB_PATH"])) {
        $arTabs["additional-tab"]["include"][] = "new_tabs.php";
        $arTabs["additional-tab"]["name"] = ($arParams["ADDITIONAL_TAB_NAME"]?$arParams["ADDITIONAL_TAB_NAME"]:GetMessage("ADDITIONAL_TAB_NAME_DEFAULT"));
        $arTabs["additional-tab"]["class"] = "";
    }

    if ($arParams['USE_GIFTS_DETAIL'] == 'Y') {
        $arTabs["gift-tab"]["include"][] = "gift_tab.php";
        $arTabs["gift-tab"]["name"] = ($arParams["GIFTS_DETAIL_TAB_TITLE"] ? $arParams["GIFTS_DETAIL_TAB_TITLE"] : GetMessage("GIFT_TAB_NAME_DEFAULT"));
        $arTabs["gift-tab"]["class"] = "bxr-detail-gift-block";
    }
...
$bPreviewBlockShow = false;
$bPreviewTextShow = false;
foreach($arParams["PREVIEW_DETAIL_PROPERTY_CODE"] as $key => $v)
{
    $v = str_replace("PROPERTY_", "", $v);
    $arParams["PREVIEW_DETAIL_PROPERTY_CODE"][$key] = $v;
    if (in_array($v, array_keys($arResult["DISPLAY_PROPERTIES"]))) $bPreviewBlockShow = true;
}
if (($arResult["PREVIEW_TEXT"] && $arParams["DISPLAY_PREVIEW_TEXT_MODE"]=="S")
        || ($arResult["PREVIEW_TEXT"] && !$arResult["DETAIL_TEXT"] && $arParams["DISPLAY_PREVIEW_TEXT_MODE"]=="E"))
{
    $bPreviewTextShow = true;
}
$arResult["PreviewTextShow"] = $bPreviewTextShow;
$arResult["PreviewBlockShow"] = ($bPreviewBlockShow || $bPreviewTextShow);
