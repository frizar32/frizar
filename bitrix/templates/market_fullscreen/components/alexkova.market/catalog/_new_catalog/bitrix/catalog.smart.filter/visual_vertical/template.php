...
{*/
?>
<div class="bx_filter bx_filter_vertical <?=$templateData["TEMPLATE_CLASS"]?> filter-mobail-js show_block">
	<div class="filter-mobile__header">
		<div class="filter-mobile__return js-filter-mobile-toggle">
		    <i class="far fa-angle-right_filter filter-mobile__arrow"></i>
		</div>
		<div class="filter-mobile__title">Фильтр</div>
	</div>
	<div class="bx_filter_section g-filter">
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
			...
			{
                                if(in_array($arItem["CODE"],$iconPropCodes) || !in_array($arItem["CODE"],$arParams['FILTER_PRICE_CODE']))
				...
				if(isset($arItem["PRICE"])):
					...
					if($arItem["VALUES"]["MIN"]["HTML_VALUE"]) {
						$arItem["VALUES"]["MIN"]["HTML_VALUE"] = ($arItem["VALUES"]["MIN"]["HTML_VALUE"]*120)/100;
					}
 
					if($arItem["VALUES"]["MAX"]["HTML_VALUE"]) {
						$arItem["VALUES"]["MAX"]["HTML_VALUE"] = ($arItem["VALUES"]["MAX"]["HTML_VALUE"]*120)/100;
					}
					?>
					<div class="bx_filter_parameters_box active">
						<span class="bx_filter_container_modef"></span>
						<div class="bx_filter_parameters_box_title" onclick="smartFilter.hideFilterProps(this)" data-role="prop_angle" ><?=$arItem["NAME"]?></div>
						<div class="bx_filter_block" data-role="bx_filter_block">
							<div class="bx_filter_parameters_box_container">
								...
								<div class="bx_ui_slider_track" id="drag_track_<?=$key?>">
									<?
									$price1 = $arItem["VALUES"]["MIN"]["VALUE"]; $price1 = floor($price1);
									$price2 = $arItem["VALUES"]["MIN"]["VALUE"] + round(($arItem["VALUES"]["MAX"]["VALUE"] - $arItem["VALUES"]["MIN"]["VALUE"])/4); $price2 = floor($price2);
									$price3 = $arItem["VALUES"]["MIN"]["VALUE"] + round(($arItem["VALUES"]["MAX"]["VALUE"] - $arItem["VALUES"]["MIN"]["VALUE"])/2); $price3 = floor($price3);
									$price4 = $arItem["VALUES"]["MIN"]["VALUE"] + round((($arItem["VALUES"]["MAX"]["VALUE"] - $arItem["VALUES"]["MIN"]["VALUE"])*3)/4); $price4 = floor($price4);
									$price5 = $arItem["VALUES"]["MAX"]["VALUE"]; $price5 = ceil($price5);

									$price1 = round($price1*120/100);
									$price2 = round($price2*120/100);
									$price3 = round($price3*120/100);
									$price4 = round($price4*120/100);
									$price5 = round($price5*120/100);
									?>
									...
									<div class="bx_ui_slider_pricebar_V bx_ui_slider_range"  style="left: 0;right: 0;" id="colorAvailableActive_<?=$key?>"></div>
			...
			foreach($arResult["ITEMS"] as $key=>$arItem)
			{
				if(
					empty($arItem["VALUES"])
					|| isset($arItem["PRICE"])
					|| in_array($arItem["CODE"],$iconPropCodes)
				)
					continue;
				if (
					$arItem["DISPLAY_TYPE"] == "A"
					&& (
						$arItem["VALUES"]["MIN"]["VALUE"] === false
						|| $arItem["VALUES"]["MAX"]["VALUE"] === false
									...
									<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label" for="<? echo $ar["CONTROL_ID"] ?>">
										<span class="bx_filter_input_checkbox <? echo $ar["DISABLED"] ? 'disabled': '' ?>">
											<input
												...
												onclick="smartFilter.click(this)"
											/>
											<span class="bx_filter_param_text"><? echo $ar["VALUE"]; ?><?
                                                                                        if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
                                                                                                ?>&nbsp;(<span data-role="count_<?=$ar["CONTROL_ID"]?>"><? echo $ar["ELEMENT_COUNT"]; ?></span>)<?
                                                                                        endif;?></span>
										</span>
				...
				<div class="bx_filter_block">
					...
					<input class="bxr-color-button" type="submit" id="set_filter" name="set_filter" value="<?=GetMessage("CT_BCSF_SET_FILTER")?>" />
						<div class="bx_filter_popup_result right" id="modef" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?> style="display: inline-block;">
							...
							<a class="bxr-color-button bxr-color-button-small" href="<?echo $arResult["FILTER_URL"]?>"><?echo GetMessage("CT_BCSF_FILTER_SHOW")?></a>
						</div>
					</div>
				</div>
			</div>
	...
	if($f == 1)
		echo '<style>.bx_filter_section.g-filter {display: none;}</style>';
	?>
