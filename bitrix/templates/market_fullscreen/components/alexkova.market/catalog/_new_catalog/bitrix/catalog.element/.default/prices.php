...
if (($usePriceCount || count($arResult["PRICES"]) > 0) && count($arResult["OFFERS"]) <= 0){
                            ...
                            if (in_array($priceIndex, $arResult["PRICES_ALLOW"])):?>
                                <div class="bxr-market-item-price bxr-format-price
                                    ...
                                    ?><span class="bxr-market-current-price bxr-market-format-price" id="<? echo $arItemIDs['PRICE']; ?>">
                                      <?=CurrencyFormat($price[$itemRangeKey]['DISCOUNT_PRICE'],$price[$itemRangeKey]['CURRENCY']) ?>
                                      <?=($showMeasure?GetMessage("CATALOG_MEASURE_DELIMITER").$arResult["CATALOG_MEASURE_RATIO"].$arResult["CATALOG_MEASURE_NAME"]:'')?>
                                    </span>
                                    <?
									//--old price--
                                    if ($boolDiscountShow && $price[$itemRangeKey]['PRICE'] != $price[$itemRangeKey]['DISCOUNT_PRICE']) {?>
                                        <span class="bxr-market-old-price" id="<? echo $arItemIDs['OLD_PRICE']; ?>">
                                          <?=CurrencyFormat($price[$itemRangeKey]['PRICE'],$price[$itemRangeKey]['CURRENCY'])?></span>
                                    <?}?>
                        ...
                        <?}
                          foreach ($price as $priceRangeIndex => $priceRange) {?>
                                <div class="bxr-market-item-price bxr-format-price <?if (count($arResult["PRICE_MATRIX"]["ROWS"]) == 1) echo "bxr-no-range-price"?>
                                    ...
                                    //--next blocks has float right--
                                    //--current price with all discounts--
                                    ?><span class="bxr-market-current-price bxr-market-format-price" id="<? echo $arItemIDs['PRICE'].'_'.$priceIndex; ?>">
                                      <?=CurrencyFormat($priceRange['DISCOUNT_PRICE'],$priceRange['CURRENCY'])?>
                                      <?=($showMeasure?GetMessage("CATALOG_MEASURE_DELIMITER").$arResult["CATALOG_MEASURE_RATIO"].$arResult["CATALOG_MEASURE_NAME"]:'')?></span>
                                    <?//--old price--
                                    if ($boolDiscountShow && $priceRange['PRICE'] != $priceRange['DISCOUNT_PRICE']) {?>
                                        <span class="bxr-market-old-price" id="<? echo $arItemIDs['OLD_PRICE'].'_'.$priceIndex; ?>">
                                          <?=CurrencyFormat($priceRange['PRICE'],$priceRange['CURRENCY'])?>
                                        </span>
                                    <?}?>
                                    <div class="clearfix"></div>
                                </div>
            ...
            if (in_array($arResult["CAT_PRICES"][$priceCode]["ID"], $arResult["PRICES_ALLOW"])):?>
                <div class="bxr-market-item-price <?if (!$priceNameShow || count($arResult["PRICES"]) == 1) echo 'bxr-market-price-without-name'?>">
                    ...
                    <span class="bxr-market-current-price" id="<? echo $arItemIDs['PRICE']; ?>">
                        <span class="price_value"><?= setRubSymbol(CurrencyFormat($arPrice['DISCOUNT_VALUE'],$arPrice["CURRENCY"]))?></span>
                        <span class="price_measure"><?=($showMeasure?GetMessage("CATALOG_MEASURE_DELIMITER").$arResult["CATALOG_MEASURE_RATIO"].$arResult["CATALOG_MEASURE_NAME"]:'')?></span>
                    </span>
                    <?//--old price--
 ...
 } elseif (is_countable($arResult["OFFERS"]) && count($arResult["OFFERS"]) > 0) {?>
