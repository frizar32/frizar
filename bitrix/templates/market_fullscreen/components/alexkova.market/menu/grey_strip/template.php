...
<div class="hidden-sm hidden-xs<?if($APPLICATION->GetCurPage(true) != SITE_DIR.'index.php'):?> container <?endif;?>">


  <nav>
    <ul data-style-menu="<?=$arParams['STYLE_MENU']?>" data-style-menu-hover="<?=$arParams['STYLE_MENU_HOVER']?>"  class="main bxr-flex-menu  <?=$classUl;?> bxr-top-menu">
<?
		//почему то менюха каталога в первом пункте
		$arResult["TREE"][1]["CHILDREN"] = $arResult["TREE"][0]["CHILDREN"];
		unset($arResult["TREE"][0]["CHILDREN"]);
        ...
        <?if($arItem["TEXT"] == 'Главная' || $arItem["TEXT"] == 'Каталог') continue;?>
            <?
                $isCurPage = ($APPLICATION->GetCurPage(false) == $arItem["LINK"] ? true : false);
                $isChildren = false;
                $glyphicon = "";
            ...
            <li class="<?=$classLi . " " . $classLiParent;?> <?if($arItem['SELECTED'] == 1) echo $classLiSelected;?>">
				...
				<?if($arItem["PARAMS"]["NO_LINK"] == "Y"):?>
					<span class="link"><?=$arItem["TEXT"].$glyphicon;?></span>
				<?else:?>
					<a class="link" <?if(!$isCurPage):?>href="<?=$arItem["LINK"]?>"<?endif?>><?=$arItem["TEXT"].$glyphicon;?></a>
				<?endif;?>
					<?if($isChildren):?>
                    <?
