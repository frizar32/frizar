	...
	function footerPrintMenuCol($items, $options)
	{
		...
		<ul <?if($addUlClass) echo "class='{$addUlClass}'"?>>
			...
			foreach($items as $item):
				$isCurPage = ($APPLICATION->GetCurPage(false) == $item["LINK"] ? true : false);
				if($maxLevel == 1 && $item["DEPTH_LEVEL"] > 1)
					continue;
				?>
				<?if($item["SELECTED"]):?>
				<li><a <?if(!$isCurPage):?>href="<?=$item["LINK"]?>" <?endif;?>class="selected"><?=$item["TEXT"]?></a></li>
			<?else:?>
				<li><a <?if(!$isCurPage):?>href="<?=$item["LINK"]?>"<?endif;?>><?=$item["TEXT"]?></a></li>
			<?endif?>
