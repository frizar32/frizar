...
if (!empty($arResult['ITEMS']))
{
	...
	?>
        <div class="h2 bold" style="font-size: 19px;"><?=$arParams["BLOCK_TITLE"]?></div>
	<div class="row bxr-list" id="mobailSlider">
	<div>
	<?
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		?>
            <div id="<?=$strMainID?>" class="t_<?=$unicumID?> col-lg-<?=$arParams["BXREADY_LIST_LG_CNT"]?> col-md-<?=$arParams["BXREADY_LIST_MD_CNT"]?> col-sm-<?=$arParams["BXREADY_LIST_SM_CNT"]?> col-xs-<?=$arParams["BXREADY_LIST_XS_CNT"]?> item">
                    <?
...
<script type="text/javascript">
	if($(window).width() <= 991){
		$('#mobailSlider').addClass('slider').addClass('slider1');
		$('#mobailSlider').children("div").addClass('sliderContent');
