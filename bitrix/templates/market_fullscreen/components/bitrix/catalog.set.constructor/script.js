...
function showCatalogSetAdd2BasketPopup(setMainPictureUrl, element_id)
{
	...
	if(setMainPictureUrl !== "" && setMainPictureUrl !== NULL){
		main_pic_wrap = '<img src="'+setMainPictureUrl+'"/>';
	} else {
		main_pic_wrap = '';
	}
	var popup = BX.PopupWindowManager.create("CatalogSetAdd2Basket"+element_id, null, {
        ...
        titleBar: {content: BX.create("span", {html: "<div>"+BX.message('AddBasketTitle')+"</div>"})},
		...
		content: '' +
			...
			'<a class="bxr-color-button" href="'+BX.message("setButtonBuyUrl")+'"><span class="bx_icon_cart"></span><span>'+BX.message("setButtonBuyName")+'</span></a>'+
			'</div>'
	});
