...
<?if (count($arResult["ITEMS"])<=0) return;?>
    ...
    <?foreach ($arResult["ITEMS"] as $key => $item):?>
        ...
        <div class="container <?=$location;?>"
            alt="<?=$item["NAME"]?>" title="<?=$item["NAME"]?>" <?/*?>style="background: url(<?=$item["DETAIL_PICTURE"]["SRC"]?>)"<?*/?>>
            <?if(isset($item["PROPERTIES"]["LINK"]["VALUE"][0])):?><a href="<?=$item["PROPERTIES"]["LINK"]["VALUE"][0];?>"><?endif;?>
