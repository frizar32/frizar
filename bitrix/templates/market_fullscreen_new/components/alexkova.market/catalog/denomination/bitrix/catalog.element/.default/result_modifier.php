...
$productSlider = getSliderForItem($arResult, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER']);
...
//if (count($productSlider) > 0 && count($productSlider[0])) {
    $arResult['MORE_PHOTO'] = $productSlider;
//} else {
//    $arResult['MORE_PHOTO'][0]["SRC"] = $elementDraw->getDefaultImage();
//}
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		$offerSlider = getSliderForItem($arOffer, $arParams['OFFER_ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER']);
		if (empty($offerSlider))
		{
                    $offerSlider = $productSlider;
		} else {
                    //add offers photo to general slider
                    if(!isset($arParams["ADD_DETAIL_TO_SLIDER_SKU"]) || $arParams["ADD_DETAIL_TO_SLIDER_SKU"] == "Y" )
