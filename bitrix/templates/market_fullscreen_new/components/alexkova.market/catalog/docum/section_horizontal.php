...
$elementBlock = 'system#ecommerce_pdf';
$elementList  = 'system#ecommerce_pdf_list';
$elementTable = 'system#ecommerce_pdf_table';
...
if ($isSidebar):?>
	<div class="col-md-3 col-sm-4 hidden-xs">
		<div class="bx-sidebar-block">
            <? if ($isMenu):
                $APPLICATION->IncludeComponent(
                    ...
                    array(
                        "ROOT_MENU_TYPE"        => isset($arParams["ROOT_MENU_TYPE"]) ? $arParams["ROOT_MENU_TYPE"]
                            : "left",
                        "MAX_LEVEL"             => isset($arParams["MAX_LEVEL"]) ? $arParams["MAX_LEVEL"] : "2",
                        "CHILD_MENU_TYPE"       => isset($arParams["CHILD_MENU_TYPE"]) ? $arParams["CHILD_MENU_TYPE"]
                            : "left",
                        ...
                        "STYLE_MENU"            => isset($arParams["STYLE_MENU"]) ? $arParams["STYLE_MENU"]
                            : "colored_light",
                        "PICTURE_SECTION"       => isset($arParams["PICTURE_SECTION"]) ? $arParams["PICTURE_SECTION"]
                            : "N",
                        "STYLE_MENU_HOVER"      => isset($arParams["STYLE_MENU_HOVER"]) ? $arParams["STYLE_MENU_HOVER"]
                            : "colored_light",
        ...
        if (CModule::IncludeModule('catalog')):
            ...
            $viewedIterator = Bitrix\Catalog\CatalogViewedProductTable::GetList(
                array(
                    ...
                    'filter' => array('FUSER_ID'    => $basketUserId,
                                      'SITE_ID'     => $siteId,
                                      '!PRODUCT_ID' => $GLOBALS["CURRENT_ELEMENT_ID"]),
        ...
        foreach ($blockSort as $sortBlock => $blockType) {
            if ($blockType == "viewed") {
                if ($viewedProductIds
                    && (!isset($arParams['VIEWED_PRODUCTS_SHOW'])
                        || $arParams['VIEWED_PRODUCTS_SHOW']
                           != 'N')
                    && $arParams["VIEWED_PRODUCTS_WERE_SHOW"] == "left"
                ...
                if (!empty($arRecomData)) {
                    if ((!isset($arParams['USE_SALE_BESTSELLERS']) || $arParams['USE_SALE_BESTSELLERS'] != 'N')
                        && $arParams['BESTSALLERS_WERE_SHOW'] == "left"
...
<? endif ?>
<div class="<?=(($isSidebar) ? "col-md-9 col-sm-8" : "col-md-12 col-sm-12 col-xs-12")?>">
	...
	<div class="row">
		...
		<div class="col-xs-12">
			<div class="<?
            if (!isset($arParams["HIDE_FILTER_MOBILE"]) || $arParams["HIDE_FILTER_MOBILE"] == "Y") echo "hidden-xs"; ?>"
			>
                <?
                $APPLICATION->IncludeComponent(
            ...
            if ($sectionDesc && $arParams["SHOW_SECTION_DESC"] == "top"
                && (!isset($_GET['PAGEN_1'])
                    || intval(
                           $_GET['PAGEN_1']
                       ) <= 1)):?>
            ...
            if ($showElementsFilters):
                $APPLICATION->IncludeComponent(
                    ...
                    array(
                        ...
                        "CATALOG_DEFAULT_SORT_ORDER" => isset($arParams['CATALOG_DEFAULT_SORT_ORDER'])
                            ? $arParams['CATALOG_DEFAULT_SORT_ORDER'] : 'desc',
                    ),
                    false,
                    array(
                        "HIDE_ICONS" => "Y"
            ...
            if ($managment_element_mode == "Y") {
                if ($elementLibrary == $elementBlock) {
                    ...
                    if (strlen($ownOptElementLib) > 0) {
                        ...
                        $optElementLib = COption::GetOptionString(
                            $module_id,
                            "catalog_list_element_type_list_" . SITE_TEMPLATE_ID,
                            $elementList
                        );
                    }
                    ...
                    if (strlen($ownOptElementLib) > 0) {
                        ...
                        $optElementLib = COption::GetOptionString(
                            $module_id,
                            "catalog_list_element_type_table_" . SITE_TEMPLATE_ID,
                            $elementTable
                        );
                    }
                };
	...
	array(
		...
		"BXREADY_ELEMENT_DRAW" => "system#brand.list.v1",
	...
	array(
        ...
        if ($sectionDesc && $arParams["SHOW_SECTION_DESC"] == "bottom"
            && (!isset($_GET['PAGEN_1'])
                || intval(
                       $_GET['PAGEN_1']
                   ) <= 1)):?>
        ...
        if (strlen($seoText) > 0 && $showSeo && (!isset($_GET['PAGEN_1']) || intval($_GET['PAGEN_1']) <= 1)):?>
			<div class="row">
				<div class="col-lg-12 tb20"><?=$seoText?></div>
			</div>
        <?endif;
            ...
            if ($blockType == "viewed") {
                if ($viewedProductIds
                    && (!isset($arParams['VIEWED_PRODUCTS_SHOW'])
                        || $arParams['VIEWED_PRODUCTS_SHOW']
                           != 'N')
                    && $arParams["VIEWED_PRODUCTS_WERE_SHOW"] == "bottom"
                    && ModuleManager::isModuleInstalled("catalog")):
                    global $viewedFilter;
                    $viewedFilter = array(
                        "ID" => $viewedProductIds
                ...
                if (!empty($arRecomData)) {
                    if ((!isset($arParams['USE_SALE_BESTSELLERS']) || $arParams['USE_SALE_BESTSELLERS'] != 'N')
                        && $arParams['BESTSALLERS_WERE_SHOW'] == "bottom"
                        && ModuleManager::isModuleInstalled("sale")) {
                ...
                if (!isset($arParams['USE_BIG_DATA']) || $arParams['USE_BIG_DATA'] != 'N') {
                    ...
                    $APPLICATION->IncludeComponent(
                        ...
                        array(
                            ...
                            "PROPERTY_CODE_"
                            . $arParams["IBLOCK_ID"]                              => $arParams["LIST_PROPERTY_CODE"],
                            "PROPERTY_CODE_"
                            . $arRecomData['OFFER_IBLOCK_ID']                     => $arParams["LIST_OFFERS_PROPERTY_CODE"],
                            "CART_PROPERTIES_"
                            . $arParams["IBLOCK_ID"]                              => $arParams["PRODUCT_PROPERTIES"],
                            "CART_PROPERTIES_"
                            . $arRecomData['OFFER_IBLOCK_ID']                     => $arParams["OFFERS_CART_PROPERTIES"],
                            "ADDITIONAL_PICT_PROP_" . $arParams["IBLOCK_ID"]      => $arParams['ADD_PICT_PROP'],
                            "ADDITIONAL_PICT_PROP_"
                            . $arRecomData['OFFER_IBLOCK_ID']                     => $arParams['OFFER_ADD_PICT_PROP'],
                            "OFFER_TREE_PROPS_" . $arRecomData['OFFER_IBLOCK_ID'] => $arParams["OFFER_TREE_PROPS"],
