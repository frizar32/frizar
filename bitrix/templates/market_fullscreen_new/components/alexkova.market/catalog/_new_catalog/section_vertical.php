...
if ($isSidebar):?>
<div class="col-md-3 hidden-sm <?if(!isset($arParams["HIDE_FILTER_MOBILE"]) || $arParams["HIDE_FILTER_MOBILE"]=="Y") echo "hidden-xs";?> ">
    ...
    if ($isFilter && $showElementsFilters && (
        preg_match("/metchiki/", $APPLICATION->GetCurPage(false)) ||
        preg_match("/narezanie_rezby/", $APPLICATION->GetCurPage(false)) ||
        preg_match("/shtangentsirkuli_mitutoyo_yaponiya/", $APPLICATION->GetCurPage(false))
    )):
        ?><p>&nbsp;</p>
        <?

        /* не забыть разобраться почему фильтр по цене не работает и заменить прег матч на условие по cnt*/
        /*$arSelect = Array("ID", "IBLOCK_ID", "CATALOG_PRICE_2");
        $arFilter = Array("IBLOCK_ID"=>$arParams["IBLOCK_ID"], "ACTIVE"=>"Y", "SECTION_ID" => $arCurSection['ID'], '!CATALOG_PRICE_2' => false);
        $cnt = CIBlockElement::GetList(Array(), $arFilter, Array(), Array(), $arSelect);*/

        if(
            preg_match("/metchiki_dlya_rezby_npsl_iz_bystrorezhushchey_stali_germaniya/", $APPLICATION->GetCurPage(false)) ||
            preg_match("/metchiki_dlya_dyuymovoy_unf_unef_unc_un_rezby_iz_bystrorezhushchey_stali_germaniya/", $APPLICATION->GetCurPage(false)) ||
            preg_match("/metchiki_dlya_rezby_pg_iz_bystrorezhushchey_stali_germaniya/", $APPLICATION->GetCurPage(false))

        ) {
            $arParams['FILTER_PRICE_CODE'][0] = 'Продажи через сайт РУБ';
        }
        ...
        global $arrFilter;
        if(isset($arrFilter['><CATALOG_PRICE_2'])) {

            $price2 = $arrFilter['><CATALOG_PRICE_2'];
            unset($arrFilter['><CATALOG_PRICE_2']);

            $arrFilter[] = array(
                    "LOGIC" => "OR",
                    "><CATALOG_PRICE_2" => $price2,
                    "><CATALOG_PRICE_3" => $price2
                    );
        }
    endif;
    ?>
<?$APPLICATION->IncludeComponent(
    ...
    array(
        ...
        "BUTTON_TEXT" => "Нужна помощь в выборе?",
        "POPUP_TITLE" => "Нужна помощь в выборе?",
...
<?endif?>
<div class="<?=(($isSidebar) ? "col-md-9 col-sm-12" : "col-xs-12")?>">
...
//          "OFFERS_LIMIT" => $arParams["LIST_OFFERS_LIMIT"],
    ...
    /* описание категории */
    $addr = $APPLICATION->GetCurDir();
    if ($sectionDesc && $arParams["SHOW_SECTION_DESC"] == "top" && (!isset($_GET['PAGEN_1']) || intval($_GET['PAGEN_1'])<=1)):?>
        <br>
        <? if(!((strpos($addr, 'filter') !== false)and(strpos($addr, 'metchiki_dlya_metricheskoy_m_rezby_iz_bystrorezhushchey_stali_guhring_germaniya') !== false))): ?>
        <div class="bxr-section-desc"><?=$sectionDesc?></div>
        <?endif;?>
    <?endif;
