        ...
        <?if (count($arResult["MORE_PHOTO"]) > 0) :
			$arWaterMark = Array(
				array(
					...
					"type" => "image",
					"size" => "real",
					"file" => $_SERVER["DOCUMENT_ROOT"].'/logo_prozr.png', // Путь к картинке
					"fill" => "exact",
				)
			);
            foreach ($arResult["MORE_PHOTO"] as $key => $val) :
                ...
                <?
                   ...
                   //$file = CFile::ResizeImageGet($val['ID'], array('width'=>563, 'height'=>582), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true); 
				   
				   $file = CFile::ResizeImageGet($val['ID'], array('width'=>563, 'height'=>582), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true, $arWaterMark); 
                ...
                <a href="<?=$val["SRC"]?>" class="fancybox" data-rel="gallery" data-fancybox-group="bx-gallery" <?if ($key == 0) echo 'id="main-photo"'?>
                    ...
                    <img src="<?=$file["src"]?>" class="zoom-img" title="<?=$imgTitle?>" alt="<?=$imgAlt?>"
