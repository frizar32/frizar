...
<?
        foreach($arResult["TREE"] as $arItem):?>
            <?
                $curItm = ($APPLICATION->GetCurPage(false) == $arItem["LINK"] ? true : false);
                $isChildren = false;
                $glyphicon = "";
                $classShow = "";
            ...
            ?>
            <li class="<?=$classLi;?> <?if($arItem['SELECTED'] == 1 || CSite::InDir($arItem["LINK"])) echo "bxr-left-menu-selected " . $classLiSelected2;?>">
                <a <?if(!$curItm):?>href="<?=$arItem["LINK"]?>"<?endif;?>><?=$s_ico;?><?=$arItem["TEXT"].$glyphicon;?></a>
                <?if($isChildren):?>
                    <ul  class="<?=$classUl2 . " " . $classShow . " " . $p_ico;?>">
                    <?foreach($arItem["CHILDREN"] as $arItemChildren):?>
                        <?$curItmChild = ($APPLICATION->GetCurPage(false) == $arItemChildren["LINK"] ? true : false);?>
                        <li class="<?=$classLi2;?> <?if($arItemChildren['SELECTED'] == 1) echo "bxr-left-menu-selected " . $classLiSelected2;?>">
                            <a <?if(!$curItmChild):?>href="<?=$arItemChildren["LINK"]?>"<?endif;?>><?=$arItemChildren["TEXT"];?></a>
                        </li>
                    <?endforeach;?>
                    </ul>
                <?endif;?>
