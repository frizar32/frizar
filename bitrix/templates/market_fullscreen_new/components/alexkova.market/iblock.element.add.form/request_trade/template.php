...
if (strlen($arResult["MESSAGE"]) > 0):?>
	...
	<?return false;?>
<?endif?>
<form id="iblockForm<?=$arParams["IBLOCK_ID"]?>" data-tpl="request_trade" name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
        <br>
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
							...
							case "N":
								...
								?>
                                    ...
                                    <?if ($inputType != 'textarea') {?>
                                        <input type="<?=$inputType?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" data-code="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"]?>"/>
                                        <?if ($inputType != 'hidden') {?>
                                            <br /><br />
                                        <?}?>
                                    <?} else {?>
                                            <textarea name="PROPERTY[<?=$propertyID?>][<?=$i?>]" rows="5" <?if($propertyID == 46):?>readonly<?endif;?> data-code="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"]?>" style="resize: none"><?=$value?></textarea><br /><br />
                                    <?}?>
			...
			<?if ($arResult['USE_PERSONAL_ACCEPT']):?>
				<br />
				<label for="accept_form_<?=$arParams["IBLOCK_ID"]?>_<?=$BXR_FORM_COUNTER?>" class="accept-form">
					<input type="checkbox" name="accept_personal" value="yes" id="accept_form_<?=$arParams["IBLOCK_ID"]?>_<?=$BXR_FORM_COUNTER?>">
					<?=$arResult['PERSONAL_SETTINGS']['CAPTION']?> <?if ($arResult['PERSONAL_SETTINGS']['ACCEPT_DOCUMENT'] && $arResult['PERSONAL_SETTINGS']['URL']) {?>[<a href="<?=$arResult['PERSONAL_SETTINGS']['URL']?>" target="_blank"><?=$arResult['PERSONAL_SETTINGS']['ACCEPT_DOCUMENT']?>
                                        </a>]<?}?>
	...
	<input <?if($arParams["HIDE_SUBMIT"] == 'Y') echo 'style="display: none;"'?> class="color-button bxr-color bxr-color-button bxr-bg-hover-light" id="submitForm_<?=$arParams["IBLOCK_ID"]?>" type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
...
<script>
	$(document).ready(function(){
		$("input[data-code=PHONE], input[data-code=USER_PHONE]").inputmask({"mask": "+7 (999) 999-9999"});
	});
</script>
