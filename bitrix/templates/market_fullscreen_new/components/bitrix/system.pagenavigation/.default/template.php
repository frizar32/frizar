...
else
{
	...
	if($_GET['PAGEN_'.$arResult["NavNum"]] > $arResult["NavPageCount"]){
		CHTTP::SetStatus("404 Not Found");
		define("ERROR_404","Y");
		header('Location: https://frizar.ru/404.php');
	}
	$sPrevHref = '';
	if ($arResult["NavPageNomer"] > 1)
	{
...
if ($arResult["bShowAll"]):
	if ($arResult["NavShowAll"]):
		?><a class="navigation-page-all" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>SHOWALL_<?=$arResult["NavNum"]?>=0"><?=GetMessage("nav_paged")?></a><?
	else:
		?><a class="navigation-page-all" href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>SHOWALL_<?=$arResult["NavNum"]?>=1"><?=GetMessage("nav_all")?></a><?
	endif;
endif;
?>
...
<?
/*if($arResult["NavPageNomer"] > 1) {
	$APPLICATION->AddHeadString('<link rel="prev" href="'.$sPrevHref.'">',true); 
}

if($arResult["NavPageNomer"] < $arResult["NavPageCount"]){
	$APPLICATION->AddHeadString('<link rel="next" href="'.$sNextHref.'">',true); 
}*/
?>
