...
class CBitrixCatalogSmartFilter extends CBitrixComponent
{
	...
	*/
	...
	function makeFilter($FILTER_NAME)
	{
		...
		$price2 = $gFilter['><CATALOG_PRICE_2'];
		...
		$resFilter = array_merge($gFilter, $arFilter);


		/* Костылим счетчик фильтра, для фильтрации по двум типам цен(на случай когда базовая не заполнена)*/
		unset($resFilter['><CATALOG_PRICE_2']);
		$resFilter[0] = array(
					"LOGIC" => "OR",
